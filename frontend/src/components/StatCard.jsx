...
export default function StatCard({ label, value, sub, delay = 0, icon, onClick }) {
  ...
  return (
    <motion.div
      ...
      {...props}
    >
      <div className="flex items-center gap-2 text-jade-200/50 text-xs mb-2">
        {icon && <span className="text-gold-500/50">{icon}</span>}
        <span>{label}</span>
      </div>
      <div className="font-number text-3xl text-jade-100 group-hover:text-gold-400 transition-colors duration-500">
